...
for l in left_cheek:
    print(f'{l}:{RIGHT_CHEEK[left_cheek.index(l)]}:{left_cheek.index(l)}')


        
    

left_cheek_batches = list(batched(left_cheek,len(left_cheek)//2))
right_cheek_batches = list(batched(RIGHT_CHEEK,len(RIGHT_CHEEK)//2))

eyes_batches = list(batched(eyes,len(eyes)//8))
eyes_batches_a = eyes_batches[0]

print(left_cheek_batches)
left_cheek_batches_a = left_cheek_batches[1]
right_cheek_batches_a = right_cheek_batches[1]
print(left_cheek_batches_a)

print(len(eyes_batches))


emoticons_list = []
for e1 in eyes:
    for e2 in eyes_batches_a:
        for m in mouths:

            for c in left_cheek_batches_a:
                string = f'{c}{e1}{m}{e2}{right_cheek_batches_a[left_cheek_batches_a.index(c)]}'
...
for i in classic_emoticons_list_of_lists:
    if classic_emoticons_list_of_lists.index(i) > 442*2 and classic_emoticons_list_of_lists.index(i) < 442*3:
        CHAR_GROUPS.append((f"Es{classic_emoticons_list_of_lists.index(i)}+1",i))


print(len(classic_emoticons_list_of_lists))
...
class KeypadApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="⌨  Emojikeypad Extension 35",
                 bg=BG, fg="#66eee7",
