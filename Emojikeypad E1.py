...
short_brow = ['>','<',')',')',']','[','~',",'"]
short_moustache = ['{','}',']','[','(',')']
short_hat_base = left_cheek + RIGHT_CHEEK +['/','>']
short_hat_tops = ['o','O','0','c','C','<','=']
short_hats = []
def get_all_ascii_characters():
    """Returns a list of every ASCII character (0-127)."""
    return [chr(i) for i in range(32,127)]
temp_list = []
ascii_chrs = get_all_ascii_characters()
short_hat_tops_crude = short_hat_tops
for i in left_cheek + RIGHT_CHEEK + short_hat_tops_crude:
    for a in ascii_chrs:
        short_hat_tops.append(f'{i}{a}')
        temp_list.append(f'{i}{a}')
        
        
short_hat_decors = ['+','-','=','"','<','>','*','_','c','C','o','O','0','`','.',',']

for t in temp_list:
    for s in short_hat_decors:
        short_hat_tops.append(f'{s}{t}')
        
for s in short_hat_tops:
    for b in short_hat_base:#
        short_hats.append(f'{s}{b}')
        
print(len(short_hats))
...
for s in simple_emoticon_lists:
    CHAR_GROUPS.append((f'HAT_{simple_emoticon_lists.index(s)+1}',list(s)))
...
class KeypadApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="⌨  Emojikeypad Extension 1 / hat keyboard v1",
                 bg=BG, fg="#66eee7",
