...
eyes2 = ['O','o','0','@','-','=','*','I','i','!','¬','`','#','~','^','$','<','>',"'",'"','x','X','u','U','T','v','V','£','Y','b']


eyes2 = list('గచఛటఠడఢథదధనరల౧౷ಈಕಗಘಙಠಡಢಥದಧನಪಫರಱಲಳಅಆಷಸ೪')


eyes2 += list('఼ాిీెేైొోౌ్ౕౖౢౣೆಿ಼ೢೣ')
left_cheek = ['[','(','{','|']
RIGHT_CHEEK = [']',')','}','|']
...
for i in classic_emoticons_list_of_lists:
    if classic_emoticons_list_of_lists.index(i) <443:
        CHAR_GROUPS.append((f"E{classic_emoticons_list_of_lists.index(i)+5680}",i))
...
class KeypadApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="⌨  Emojikeypad Extension 17",
                 bg=BG, fg="#66eee7",
