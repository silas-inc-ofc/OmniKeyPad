...
eyes = ['O','o','0','@','-','=','*','I','i','!','¬','`','#','~','^','$','<','>',"'",'"','x','X','u','U','T','v','V','£','Y','b']
mouths = ['O','o','0','.','_','u','U','x','X','w','W','3','__']

left_cheek = ['[','(','{','|']
RIGHT_CHEEK = [']',')','}','|']

short_eyes = [':',';','=','B','X','8','0','o','O','!','*','.',',','%']
short_nose = ['-','¬','=','^','o','O','0','@']
short_mouth = ['D','d','b','3','o','O','p','P','B','c','C','x','X','s','S','/','E','l','L','j','£','(',')','<','>','|','[',']','{','}','$','t','@','Z',]
short_brow = ['>','<',')',')',']','[','~',",'"]
short_moustache = ['{','}',']','[','(',')']
...
for e1 in eyes:
    for e2 in eyes:
        for m in mouths:
            ...
            for c in left_cheek:
                string = f'{c}{e1}{m}{e2}{RIGHT_CHEEK[left_cheek.index(c)]}'
            
                emoticons_list.append(string)
                
                
short_emoticons_list = []
for e in short_eyes:
    for m in short_mouth:
        string = f'{e}{m}'
        short_emoticons_list.append(string)
        for s in short_moustache:
            short_emoticons_list.append(f'{e}{s}{m}')
            
        
        
        for b in short_brow:
            string2 = f'{b}{e}{m}'
            short_emoticons_list.append(string2)
            for s in short_moustache:
                string2a = f'{b}{e}{s}{m}'
                short_emoticons_list.append(string2a)
        
        for n in short_nose:
            string3 = f'{e}{n}{m}'
            short_emoticons_list.append(string3)
            for s in short_moustache:
                string3a = f'{e}{n}{s}{m}'
                short_emoticons_list.append(string3a)
        
            for b in short_brow:
                string4 = f'{b}{e}{n}{m}'
                short_emoticons_list.append(string4)
                for s in short_moustache:
                    string4a = f'{b}{e}{n}{s}{m}'
                    short_emoticons_list.append(string4a)
...
simple_emoticon_lists = list(batched(short_emoticons_list,3000))

a = 0
for d in simple_emoticon_lists:
    a += len(list(d))
...
E1 = emoticons_list[:2000]
E2 = emoticons_list[2000:4000]
E3 = emoticons_list[4000:6000]
E4 = emoticons_list[6000:8000]
E5 = emoticons_list[8000:10000]
E6 = emoticons_list[10000:12000]
E7 = emoticons_list[12000:14000]
E8 = emoticons_list[14000:16000]
E9 = emoticons_list[16000:18000]
E10 = emoticons_list[18000:20000]
E11 = emoticons_list[20000:22000]
E12 = emoticons_list[22000:24000]
E13 = emoticons_list[24000:]

classic_emoticons_list_of_lists = [E1,E2,E3,E4,E5,E6,E7,E8,E9,E10,E11,E12,E13]
...
for i in classic_emoticons_list_of_lists:
    CHAR_GROUPS.append((f"E{classic_emoticons_list_of_lists.index(i)+1}",i))


for s in simple_emoticon_lists:
    CHAR_GROUPS.append((f'S_E{simple_emoticon_lists.index(s)+1}',list(s)))
...
class KeypadApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="⌨  Emojikeypad Version 2",
                 bg=BG, fg="#66eee7",
