...
eyes += list('గచఛటఠడఢథదధనరల౧౷ಈಕಗಘಙಠಡಢಥದಧನಪಫರಱಲಳಅಆಷಸ೪')


eyes += list('఼ాిీెేైొోౌ్ౕౖౢౣೆಿ಼ೢೣ')


eyes += ['O','o','0','@','-','=','*','I','i','!','¬','`','#','~','^','$','<','>',"'",'"','x','X','u','U','T','v','V','£','Y','b']
eyes += list('∅ↀ∂ↇↁ∆ↈↂ∇∼∿≂∺∽≀≃≄≇≊≍≐≓≅≈≋≎≑≔≆≉≌≏≒≕≖≙≜≟≢≮≗≚≝≠≣≯≘≛≞≡≤≰≥≨≬≱≦≩≭≲≴≸≧≪≫≳≵≹≺≽⊀⊃⊆⊉≻≾⊁⊄⊇⊊≼≿⊂⊅⊈⊋⊌⊏⊒⊕⊘⊛⊍⊐⊓⊖⊙⊜⊞⊡⊤⊧⊪⊭⊟⊢⊥⊨⊫⊮⊠⊣⊦⊩⊬⊯⊰⊼⊿⊱⊴⊶⊷⊹⊺⊽⋀⊲⊵⊸⊻⊾⋁⋂⋈⋃⋏⋎⋑⋒⋄⋇⋍⋐⋓⋗⋝⋠⋣⋕⋘⋞⋡⋤⋖⋙⋜⋟⋢⋥⋦⋩⋬⋲⋵⋧⋪⋭⋳⋶⋨⋫⋴⋷⋸⋻⌖①④⑦⋹⋼⌗②⑤⑧⋺⋽③⑥⑨')
eyes2 = list('ᐁᐂᐃᐄᐅᐆᐇᐈᐉᐊᐋᐌᐍᐎᐏᐐᐑᐒᐓᐔᐕᐖᐗᐘᐙᐚᐛᐫᐬᐭᐮᐯᐰᐱᐲᐳᐴᐵᐶᐷᐸᐹᐺᐻᐼᐽᐾᐿᑀᑁᑂᑃᑄᑅᑆᑇᑈᑌᑍᑎᑏᑐᑑᑒᑓᑔᑕᑖᑗᑘᑙᑚᑛᑜᑝᑞᑟᑠᑡᑢᑣᑤᑥᑦᑧᑨᑩᑪᑬᑭᑮᑯᑰᑱᑲᑳᑴᑵᑶᑷᑸᑹᑺᑻᑼᑽᑾᑿᒀᒁᒂᒄᒅᒆᒇᒈᓀᓁᓂᓃᓄᓅᓆᓇᓈᓉᓊᓋᓌᓍᓎᓏᕞᕟᕠᕡᕢᕣᕤᕥᕦᕧᕨᕩᕪᕫᕬᕭᕮᕯᕰᕱᕲᕳᕴᕵᕶᕷᕸᕹᕺᕼᕾᕿᖀᖁᖂᖃᖄᖆᖇᖈᖉ')

mouths = list('⁔‿ϖ∆∇∏∐∩∪∧∨∸∻∾≁∼∿≂∽≍⊌⊍⊎⊓⊔⋀⋁⊳⊲⋂⋃⋒⋓⋄⨃⨆⨄⨅⨿⩂⩅⩃⩀⩂⩅⩄⩁⩇⩈⩋⩎⩑⩔⩗⩉⩌⩏⩒⩕⩘⩊⩍⩐⩓⩖⩙⩚⩝⩛⩟⪦⪧⫏⫐⪤Дд')

left_cheek =list('∈∉≮≤≰≨≦≲≴≪≺⊆≾⊊⊂≼⊄⊈⊏⊑⋠⋞⋜⋢⋦⋨⩽⪁⪇⪍⪅⪃⪉⪙⪗⪝⪕⪛⪡⪬⪯⪪⪷⪽⫃⪵⪻⫁⪹⪿⫅⫉⫇⫹⫷⩻⩹⩿')
RIGHT_CHEEK =list('∋∌≯≥≱≩≧≳≵≫≻⊇≿⊋⊃≽⊅⊉⊐⊒⋡⋟⋝⋣⋧⋩⩾⪂⪈⪎⪆⪄⪊⪚⪘⪞⪖⪜⪢⪭⪰⪫⪸⪾⫄⪶⪼⫂⪺⫀⫆⫊⫈⫺⫸⩼⩺⪀')
eyes_batches = list(batched(eyes,len(eyes)//19))
eyes_batches_1 = list(batched(eyes2,len(eyes2)//9))
print(len(eyes_batches))
print(len(eyes_batches_1))
eyes0 = eyes_batches_1[7]
eyes1 = eyes_batches[16]
...
class KeypadApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="⌨  Emojikeypad Extension 1014",
                 bg=BG, fg="#66eee7",
